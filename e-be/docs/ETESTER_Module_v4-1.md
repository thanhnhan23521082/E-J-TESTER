...
QR code:   react-qr-code — generate QR badge
JWT sign:  dùng JWT library đang có sẵn trong codebase (detect từ requirements)
```
...
backend/
├── shared/
│   ├── model.py                    # Smart Parenting (KHÔNG SỬA)
│   └── etester_models.py           # ETESTER models
├── api/etester/
│   ├── routes.py
│   ├── services.py
│   ├── claude_service.py           # 4 Claude prompts
│   └── auth_scoring.py             # 7-dimension engine
└── alembic/versions/002_etester.py

frontend/src/
├── pages/
│   ├── ETESTERDashboard.jsx        # Student view
│   ├── MentorDashboard.jsx         # Mentor view — NEW
│   └── ETESTERPublic.jsx           # QR destination
├── components/etester/
│   ├── ArtifactGraph.jsx
│   ├── MilestoneTimeline.jsx
│   ├── ArtifactForm.jsx            # Includes post-submit suggestion screen
│   ├── TraceLinkSuggestions.jsx    # Post-submit: student sees AI suggestions — NEW
│   ├── MentorPendingQueue.jsx      # Mentor review queue — NEW
│   ├── MentorTraceReview.jsx       # Single link review card — NEW
│   ├── NarrativeCard.jsx
│   ├── QRBadge.jsx
│   ├── AuthScoreCard.jsx
│   └── ContributorDonut.jsx
└── services/etesterApi.js
```
...
> **[AI]** File: `backend/api/etester/claude_service.py`
...
"""
    ...
    return parse_json(result, fallback=[])
```
...
"""
    ...
    return parse_json(result, fallback={"has_leadership": False})
```
...
> **[AI]** All in `frontend/src/components/etester/` and `frontend/src/pages/`
